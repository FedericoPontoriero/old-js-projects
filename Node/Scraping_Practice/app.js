...
async function scrapeData(url, page) {
	try {
		...
		let userScore = $('.user-score-chart').attr('data-percent');
		let imgUrl = $(
			'#main > section > div.header.large.border.first.lazyloaded > div > div > section > div.poster > div.image_content > a > img'
		).attr('src');
		...
		browser.close();
		return { title, releaseDate, overview, userScore, imgUrl, crew };
	} catch (error) {
		console.log(error);
	}
}
