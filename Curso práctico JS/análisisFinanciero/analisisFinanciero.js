...
function calculateInstalmentsAmount(
  purchaseAmount,
  instalments,
  monthlyInteresRate
) {
  const num =
    Math.pow(1 + monthlyInteresRate, instalments) *
    monthlyInteresRate *
    purchaseAmount;
...
function calculate() {
  ...
  const instalmentsAmount = calculateInstalmentsAmount(
    purchaseAmount,
    instalments,
    monthlyInteresRate
  );
  ...
  result.innerHTML = `La cuota mensual a pagar es: ${instalmentsAmount.toFixed(
    2
  )} por ${instalments} meses`;
}
